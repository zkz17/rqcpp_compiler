...
class High2MidTransLator:
    ...
    def uncomp_IfStmtNode(self, ifstmt):
        ...
        for cond, body, pre in ifstmt._branches:
            if cond:
                ...
                self.emit(MidBranchEqZ(MidVariable(cond.name(), cond.index()), label_cond_backward), label_cond_forward)

                self.uncomp(body)

                label_skip_forward, label_skip_backward = self.get_label_pair()
                self.emit(MidBranchControl(MidVariable(cond.name(), cond.index()), label_skip_backward), label_skip_forward)
                branch_back.append((MidBranchControl(MidVariable(cond.name(), cond.index()), label_skip_forward), label_skip_backward, pre))

                self.emit(MidBranchControl(MidVariable(cond.name(), cond.index()), label_cond_forward), label_cond_backward)
            else:
                self.uncomp(body)
    ...
    def uncomp_WhileStmtNode(self, whilestmt):
        ...
        counter_var = MidVariable(self.emitter.get_tempvar_name())
        self.emit(Pop(counter_var))
        
        label_cond_forward, label_cond_backward = self.get_label_pair()
        self.emit(MidBranchNeqZ(MidVariable(whilestmt._cond.name(), whilestmt._cond.index()), label_cond_backward), label_cond_forward)

        self.uncomp(whilestmt._body)
        self.emit(MidSubi(counter_var, 1))

        self.emit(MidBranchControl(MidVariable(whilestmt._cond.name(), whilestmt._cond.index()), label_cond_forward), label_cond_backward)
        self.emit(MidBranchControl(counter_var, label_counter_forward), label_counter_backward)

    def uncomp_AssignNode(self, assign):
        tempvar = MidVariable(self.emitter.get_tempvar_name())
        left = MidVariable(assign._left.name(), assign._left.index())
        if isinstance(assign._right, UnaOpNode):
            self.emit(Pop(tempvar))
            self.emit(MidSwap(tempvar, left))
            self.emit(MidArithmetic(assign._right._op.value, tempvar, MidVariable(assign._right._right.name(), assign._right._right.index())))
        elif isinstance(assign._right, BinOpNode):
            self.emit(Pop(tempvar))
            self.emit(MidSwap(tempvar, left))
            self.emit(MidArithmeticB(assign._right._op.value, tempvar, MidVariable(assign._right._left.name(), assign._right._left.index()), MidVariable(assign._right._right.name(), assign._right._right.index())))
        elif isinstance(assign._right, SingletonNode) and isinstance(assign._right._value, NumNode):
            self.emit(MidXori(left, assign._right.value()))
            self.emit(Pop(left))
        elif isinstance(assign._right, SingletonNode) and isinstance(assign._right._value, IDNode):
            self.emit(MidXor(left, MidVariable(assign._right.name(), assign._right.index())))
            self.emit(Pop(left))
    ...
    def init_params(self, proc):
        ...
        for param in reversed(proc._params):
            tempvar = MidVariable(self.emitter.get_tempvar_name())
            self.emit(Pop(tempvar))
            self.emit(MidSwap(tempvar, MidVariable(param.name())))
    ...
    def visit_WhileStmtNode(self, whilestmt):
        ...
        label_cond_forward, label_cond_backward = self.get_label_pair()
        self.emit(MidBranchEqZ(MidVariable(whilestmt._cond.name(), whilestmt._cond.index()), label_cond_backward), label_cond_forward)

        self.emit(MidAddi(counter_var, 1))
    ...
    def visit_QifStmtNode(self, qifstmt):
        qbit = MidVariable(qifstmt._qbits.name(), qifstmt._qbits._index._index.name())
    ...
    def visit_UnitaryNode(self, unitary):
        ...
        if gate['qvar'] == 1:
            ...
            if isinstance(offset, SingletonNode):
                self.emit(MidUnitary(unitary.name(), MidVariable(qreg, offset.name())))
            else:
            ...
            if isinstance(offset1, SingletonNode) and isinstance(offset2, SingletonNode):
                self.emit(MidUnitaryB(unitary.name(), MidVariable(qreg1, offset1.name()), MidVariable(qreg2, offset2.name())))
            else:
                raise Exception(f'Unrecognized index for qubit register {qreg}')
        else:
