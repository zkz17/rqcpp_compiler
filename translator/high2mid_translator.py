...
class High2MidTransLator:
    ...
    def uncomp_WhileStmtNode(self, whilestmt):
        ...
        counter_var = Variable.get_var(self.emitter.get_tempvar_name())
    ...
    def uncomp_AssignNode(self, assign):
        tempvar = Variable.get_var(self.emitter.get_tempvar_name())
        left = Variable.get_var(assign._left.name(), assign._left.index())
        if isinstance(assign._right, UnaOpNode):
            self.emit(Pop(tempvar))
            self.emit(MidSwap(tempvar, left))
            self.emit(MidArithmetic(assign._right._op.value, tempvar, Variable.get_var(assign._right._right.name(), assign._right._right.index())))
        elif isinstance(assign._right, BinOpNode):
            self.emit(Pop(tempvar))
            self.emit(MidSwap(tempvar, left))
            self.emit(MidArithmeticB(assign._right._op.value, tempvar, Variable.get_var(assign._right._left.name(), assign._right._left.index()), Variable.get_var(assign._right._right.name(), assign._right._right.index())))
        elif isinstance(assign._right, SingletonNode) and isinstance(assign._right._value, NumNode):
            self.emit(MidXori(left, Variable.get_var(assign._right.value())))
            self.emit(Pop(left))
        elif isinstance(assign._right, SingletonNode) and isinstance(assign._right._value, IDNode):
            self.emit(MidXor(left, Variable.get_var(assign._right.name(), assign._right.index())))
            self.emit(Pop(left))
    ...
    def visit_TopNode(self, topnode):
        ...
        self.emit(MidBranch(self.emitter.get_procentry_label(topnode._entry)))
    ...
    def init_params(self, proc):
        ...
        for param in reversed(proc._params):
            tempvar = Variable.get_var(self.emitter.get_tempvar_name())
            self.emit(Pop(tempvar))
            self.emit(MidSwap(tempvar, Variable.get_var(param.name())))
    ...
    def visit_QifStmtNode(self, qifstmt):
        qbit = Variable.get_var(qifstmt._qbits.name(), qifstmt._qbits._index._index.name())
    ...
    def visit_UnitaryNode(self, unitary):
        ...
        if gate['qvar'] == 1:
            ...
            if isinstance(offset, SingletonNode):
                self.emit(MidUnitary(unitary.name(), Variable.get_var(qreg, offset.name())))
            else:
            ...
            if isinstance(offset1, SingletonNode) and isinstance(offset2, SingletonNode):
                self.emit(MidUnitaryB(unitary.name(), Variable.get_var(qreg1, offset1.name()), Variable.get_var(qreg2, offset2.name())))
            else:
                raise Exception(f'Unrecognized index for qubit register {qreg}')
        else:
